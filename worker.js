...
async function fetchAndApply(request) {
  ...
  // 检查是否为禁用路径
  if (disabled_paths.includes(url.pathname)) {
    // 重定向到教程页
    return Response.redirect(tutorial_page, 302)
  }
  ...
  if (blocked_region.includes(region)) {
    ...
    if (content_type && content_type.includes('text/html') && content_type.includes('UTF-8')) {
      original_text = await replace_response_text(original_response_clone, upstream_domain, url.hostname)
    } else {
